...
function activate(context) {
	...
	let disposableRenameCommand = vscode.commands.registerCommand('mavu-dupe.startDuplicate', (clicked_file, selected_files) => {
		if (!selected_files)
			return;
		current_renaming = {
			files: []
		...
		let batchFilePath = PathModule.join(__dirname, '.mavu-dupe.txt');
			...
			current_renaming.save = function () {
				...
				if (current_renaming.files.length == new_names.length) {
					current_renaming.files.forEach((file, i) => {
						...
						fs.copyFileSync(file.fsPath, new_path);
					});
				} else {
